...
const STEPS = [
  {
    key: "nuevo", short: "Recibido", label: "Pedido recibido",
    desc: "Hemos recibido tu pedido y el pago ha sido procesado correctamente.",
    icon: (c) => (<svg width={c} height={c} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5M12 22V12"/></svg>),
  },
  {
    key: "en_preparacion", short: "Preparando", label: "En preparación",
    desc: "Nuestro equipo está preparando tu pedido con cuidado.",
    icon: (c) => (<svg width={c} height={c} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>),
  },
  {
    key: "empaquetando", short: "Embalando", label: "Empaquetando",
    desc: "Tu pedido está siendo embalado y protegido para el envío.",
    icon: (c) => (<svg width={c} height={c} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22V12m9-5-9 5-9-5m0 0 9-5 9 5"/><path d="M3 17l9 5 9-5"/></svg>),
  },
  {
    key: "listo_envio", short: "Listo", label: "Listo para envío",
    desc: "Tu pedido está listo y esperando al transportista.",
    icon: (c) => (<svg width={c} height={c} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 3v5h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>),
  },
  {
    key: "enviado", short: "En camino", label: "En camino",
    desc: "Tu pedido está en ruta. Pronto llegará a tu puerta.",
    icon: (c) => (<svg width={c} height={c} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14m-7-7 7 7-7 7"/></svg>),
  },
  {
    key: "completado", short: "Entregado", label: "Entregado",
    desc: "¡Tu pedido ha llegado! Esperamos que lo disfrutes.",
    icon: (c) => (<svg width={c} height={c} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>),
  },
]
...
function SearchBox({ onSearch, loading }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); if (val.trim()) onSearch(val.trim()) }}
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={s.key} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, flex: 1 }}>
              ...
              <p style={{
                fontSize: 10, fontWeight: 700, textAlign: "center", letterSpacing: "0.03em", lineHeight: 1.3,
  ...
  if (safeEvents.length === 0) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 0" }}>
        ...
        <div>
          <p style={{ fontSize: 13, fontWeight: 700, color: "#1e293b", margin: 0 }}>{STEPS.find(s => s.key === status)?.label ?? "Pedido recibido"}</p>
        ...
        return (
          <div key={i} style={{ display: "flex", gap: 14 }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
              ...
              {i < safeEvents.length - 1 && <div style={{ width: 1, flex: 1, background: "#f1f5f9", margin: "4px 0", minHeight: 28 }} />}
            </div>
    ...
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      <div style={{
        ...
        <p style={{ fontSize: 15, color: "#64748b", margin: "0 0 32px", lineHeight: 1.6 }}>
          Introduce tu número de seguimiento para ver<br />el estado actualizado en tiempo real.
        </p>
        <div style={{ position: "relative", zIndex: 1 }}>
          <SearchBox onSearch={onSearch} loading={loading} />
        </div>
        <p style={{ fontSize: 12, color: "#94a3b8", margin: "14px 0 0" }}>
          Formato: <span style={{ fontFamily: "monospace", fontWeight: 600, color: "#f97316" }}>TRK-XXXXXXXX</span> · Lo encontrarás en el email de confirmación
        </p>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {STEPS.map((s) => (
            <div key={s.key} style={{
              ...
              padding: "14px 12px", borderRadius: 14, background: "#fafafa", border: "1px solid #f1f5f9",
            }}>
              <div style={{
                width: 34, height: 34, borderRadius: 10, background: "#fff7ed", color: "#f97316",
...
function ResultScreen({ order, onReset, showBack }) {
  ...
  return (
    <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(12px)", transition: "opacity 0.4s ease, transform 0.4s ease" }}>
        ...
        <div style={{ padding: "28px 32px 32px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 24, marginBottom: 28 }}>
            <div>
              ...
              <h2 style={{ fontSize: 32, fontWeight: 800, color: "#0f172a", margin: "0 0 8px", letterSpacing: "-0.03em", lineHeight: 1.1 }}>{step?.label}</h2>
              <p style={{ fontSize: 14, color: "#64748b", margin: 0, lineHeight: 1.6, maxWidth: 380 }}>{step?.desc}</p>
            </div>
            <div style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              ...
              {items.map((item, idx) => (
                <div key={item.id ?? idx} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: "#1e293b", margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.product_name}</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 20, padding: "22px 24px", flex: 1 }}>
            <p style={{ fontSize: 11, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 18px" }}>Historial</p>
          ...
          {(order.address || order.city) && (
            <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 20, padding: "18px 24px" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: "#eff6ff", border: "1px solid #bfdbfe", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {/* HELP BANNER */}
      <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 20, padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: "#fff7ed", border: "1px solid #fed7aa", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <a href="mailto:hola@tienda.com" style={{
          ...
          background: "#f97316", color: "#fff", padding: "10px 20px", borderRadius: 12,
          fontSize: 13, fontWeight: 700, textDecoration: "none", transition: "background 0.2s", flexShrink: 0,
        }}
          onMouseEnter={e => e.currentTarget.style.background = "#ea580c"}
          onMouseLeave={e => e.currentTarget.style.background = "#f97316"}
        >
          Contactar <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14m-7-7 7 7-7 7"/></svg>
        </a>
      </div>
    </div>
  )
...
function NotFound({ id, onReset, onRetry }) {
  return (
    <div style={{ maxWidth: 480 }}>
      <div style={{ background: "#fff", border: "1px solid #fee2e2", borderRadius: 24, padding: "48px 40px", textAlign: "center", marginBottom: 20 }}>
        ...
        <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: "0 0 8px", letterSpacing: "-0.02em" }}>Pedido no encontrado</h2>
        <p style={{ fontSize: 14, color: "#64748b", margin: "0 0 4px", lineHeight: 1.6 }}>No hemos encontrado ningún pedido con el número</p>
        <p style={{ fontSize: 15, fontWeight: 700, color: "#f97316", margin: "0 0 24px", fontFamily: "monospace" }}>{id}</p>
        <p style={{ fontSize: 12, color: "#94a3b8", margin: 0, lineHeight: 1.6 }}>
          El número de seguimiento tiene el formato <span style={{ fontFamily: "monospace", color: "#f97316" }}>TRK-XXXXXXXX</span>. Lo encontrarás en el email de confirmación.
        </p>
      </div>
...
function ErrorScreen({ id, onReset, onRetry }) {
  return (
    <div style={{ maxWidth: 440 }}>
      <div style={{ background: "#fff", border: "1px solid #fed7aa", borderRadius: 24, padding: "48px 40px", textAlign: "center", marginBottom: 20 }}>
        <div style={{ width: 64, height: 64, borderRadius: 18, background: "#fff7ed", border: "1px solid #fed7aa", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
        ...
        <p style={{ fontSize: 14, color: "#64748b", margin: "0 0 28px", lineHeight: 1.6 }}>No hemos podido obtener la información de tu pedido. Por favor, inténtalo de nuevo.</p>
        <button onClick={() => onRetry(id)} style={{ width: "100%", height: 48, background: "#f97316", border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "background 0.2s" }}
          onMouseEnter={e => e.currentTarget.style.background = "#ea580c"}
          onMouseLeave={e => e.currentTarget.style.background = "#f97316"}
        >Reintentar</button>
      </div>
...
export default function OrderTracking() {
  ...
  async function search(tracking) {
    ...
    setQid(tracking)
    try {
      const res = await fetchOrder(tracking)
